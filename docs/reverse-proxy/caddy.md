...
> ([yarp.md](yarp.md), [ADR-0017](../decisions/0017-in-process-yarp-proxy.md)). It is kept, supported
...
Before [ADR-0017](../decisions/0017-in-process-yarp-proxy.md) this was the default, so an instance that
...
is the built-in provider since ADR-0017) and either flip **Settings → Reverse proxy** in the UI
...
environment:
  ...
  WATCHTOWER__PROXY__PROVIDER: caddy                # required since ADR-0017; the default is yarp
