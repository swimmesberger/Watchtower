...
public sealed class YarpListenerState {
    ...
    /// <inheritdoc cref="YarpListenerSnapshot.PortRoutePorts"/>
    public IReadOnlySet<int> PortRoutePorts => Current.PortRoutePorts;
}
...
public sealed record YarpListenerSnapshot {
    ...
    private readonly FrozenSet<int> _portRoutePorts = FrozenSet<int>.Empty;
    ...
    public IReadOnlySet<int> PortRoutePorts {
        get => _portRoutePorts;
        init => _portRoutePorts = value as FrozenSet<int> ?? value?.ToFrozenSet() ?? FrozenSet<int>.Empty;
    }
}
