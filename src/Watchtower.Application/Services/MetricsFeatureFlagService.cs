...
/// sqlite and influxdb backends, ADR-0013). Evaluated per <c>elarion.session</c> call against the routed
/// source, so it follows a runtime backend switch on the next snapshot fetch; runtime-variable
/// availability (e.g. Docker unreachable) stays on the data as <c>available</c>/<c>reason</c>. Unknown
/// names fail closed.
/// </summary>
