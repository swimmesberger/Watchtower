...
public sealed class ComposeOverrideFileTests {
    ...
    // ── Merging the device-mapping plan into the same override ───────────────────────────────────

    /// <summary>
    /// All three policies land in the one document (ADR-0030): <c>devices:</c> renders after
    /// <c>image:</c> and <c>environment:</c>, in Compose's <c>host:container[:permissions]</c> string
    /// form, and a service only the device plan names still gets its own entry.
    /// </summary>
    [Fact]
    public void Render_MergesDeviceMappingsIntoTheSameDocument() {
        var plan = EnvInjectionPlan.Create(new EnvInjectionRequest(
            [new EnvInjectionService("web", "true")], StackId: 7, AppApiToken: "wtapp_abc"));
        var devicePlan = new DeviceMappingPlan(
            [new ServiceDeviceMappings("transcoder",
                 [new ServiceDevice("/dev/dri/renderD128", "/dev/dri/renderD128", null),
                  new ServiceDevice("/dev/ttyUSB0", "/dev/ttyUSB1", "rw")]),
             new ServiceDeviceMappings("web",
                 [new ServiceDevice("/dev/fuse", "/dev/fuse", "rwm")])],
            []);
        ...
        Assert.Equal(
            ...
            """.ReplaceLineEndings("\n"),
            ComposeOverrideFile.Render(plan, imagePlan: null, devicePlan));
    }

    /// <summary>
    /// A GPU-resolved service (ADR-0031) carries the nodes' owning groups too: <c>group_add:</c>
    /// renders after <c>devices:</c>, each GID as a quoted string — an unquoted number is looked up
    /// as a group <em>name</em> inside the container by some runtimes.
    /// </summary>
    [Fact]
    public void Render_WritesGroupAddForAServiceWithGroupIds() {
        var devicePlan = new DeviceMappingPlan(
            [new ServiceDeviceMappings("transcoder",
                 [new ServiceDevice("/dev/dri/renderD128", "/dev/dri/renderD128", null)]) {
                GroupIds = [44, 105],
        ...
        Assert.Equal(
            ...
            """.ReplaceLineEndings("\n"),
            ComposeOverrideFile.Render(EnvInjectionPlan.Empty, imagePlan: null, devicePlan));
    }

    /// <summary>
    /// An NVIDIA-resolved service renders a reservation rather than a device mapping (ADR-0032):
    /// the node without the toolkit-injected user-space driver does not work, and the toolkit is
    /// asked through <c>deploy.resources.reservations.devices</c>. Compose honours that key outside
    /// Swarm, unlike most of <c>deploy:</c>.
    /// </summary>
    [Fact]
    public void Render_WritesAnNvidiaReservationWithTheVideoCapability() {
        var devicePlan = new DeviceMappingPlan(
            [new ServiceDeviceMappings("transcoder", []) { NvidiaGpus = true }],
            []);

        var rendered = ComposeOverrideFile.Render(EnvInjectionPlan.Empty, imagePlan: null, devicePlan);
        ...
        Assert.Equal(
            ...
            """.ReplaceLineEndings("\n"),
            rendered);
        // Without "video" the toolkit injects no encoder libraries and NVENC fails while CUDA works
        // — the failure this capability list exists to prevent.
        Assert.Contains("video", rendered, StringComparison.Ordinal);
        // A reservation is not a device mapping; emitting both would be a contradiction.
        Assert.DoesNotContain("devices:\n      -", rendered, StringComparison.Ordinal);
    }

    /// <summary>
    /// The ADR-0030 back-compat guarantee: a stack with no device rows renders exactly its pre-device
    /// document — an empty device plan and an absent one are the same bytes, and cannot alone produce
    /// a file.
    /// </summary>
    [Fact]
    public void Render_IsUnchangedWithoutADevicePlan() {
        var plan = EnvInjectionPlan.Create(new EnvInjectionRequest(
            [new EnvInjectionService("app")], StackId: 1, AppApiToken: "wtapp_x"));

        Assert.Equal(
            ComposeOverrideFile.Render(plan),
            ComposeOverrideFile.Render(plan, imagePlan: null, DeviceMappingPlan.Empty));
        Assert.Null(ComposeOverrideFile.Render(EnvInjectionPlan.Empty, null, DeviceMappingPlan.Empty));
    }
